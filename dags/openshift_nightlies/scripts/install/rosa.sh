...
_wait_for_nodes_ready(){
    ...
    while [ ${ITERATIONS} -le $((${NODES_COUNT}*100)) ] ; do
...
index_mgmt_cluster_stat(){
    ...
    curl -L https://github.com/cloud-bulldozer/kube-burner/releases/download/v${KUBE_BURNER_RELEASE}/kube-burner-v${KUBE_BURNER_RELEASE}-Linux-x86_64.tar.gz -o kube-burner.tar.gz
...
EOF
    ...
    kube-burner index --uuid=${UUID} --prometheus-url=${MC_PROMETHEUS} --token ${MC_PROMETHEUS_TOKEN} --start=$((START_TIME+600)) --end=$END_TIME --step 2m --metrics-profile ${METRIC_PROFILE}  --config ./baseconfig.yml --log-level debug
...
if [[ "$operation" == "install" ]]; then
    ...
    if [ $HCP == "true" ]; then _get_sc_mc_details; fi
    cleanup
    index_metadata
